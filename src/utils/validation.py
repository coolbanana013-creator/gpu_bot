...
def validate_path(
    ...
    if not isinstance(path, str):
        raise ValueError(f"{name} must be a string, got {type(path).__name__}")
    
    # Convert to Path object for safer handling
    try:
        path_obj = Path(path)
    except Exception as e:
        raise ValueError(f"{name} is not a valid path: {e}")
    
    # Check for directory traversal attempts
    if '..' in path or path.startswith('~'):
        raise ValueError(f"{name} contains invalid characters (directory traversal attempt)")
    ...
    try:
        ...
        raise ValueError(f"{name} cannot be resolved: {e}")
    
    # Check if path exists if required
    if must_exist and not resolved_path.exists():
        raise ValueError(f"{name} does not exist: {resolved_path}")
    
    # Check file extension if specified
    if allowed_extensions:
        if not any(str(resolved_path).endswith(ext) for ext in allowed_extensions):
            raise ValueError(
                f"{name} must have one of these extensions: {allowed_extensions}, "
                f"got: {resolved_path.suffix}"
            )
    
    logger.debug(f"Validated path {name}: {resolved_path}")
