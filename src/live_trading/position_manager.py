...
class PositionManager(ABC):
    ...
    def __init__(self, initial_balance: float, max_positions: int = 5):
        """Initialize position manager (FIXED: default max_positions 5 for realistic portfolio)."""
    ...
    def calculate_position_size(self, price: float, leverage: int, risk_percent: float = 0.02) -> float:
        ...
        FIXED: Cap at 25% of balance (Kelly limit)
        ...
        """
        risk_percent = min(risk_percent, 0.25)  # FIXED: Cap at 25%
    ...
    def get_maintenance_margin_rate(self, leverage: int) -> float:
        """Get tiered maintenance margin rate based on leverage (KuCoin standard)."""
        if leverage <= 5:
            return 0.004  # 0.4%
        elif leverage <= 20:
            return 0.005  # 0.5%
...
class PaperPositionManager(PositionManager):
    ...
    def __init__(self, initial_balance: float, max_positions: int = 5):
        """Initialize paper trading manager (FIXED: default max_positions 5)."""
        super().__init__(initial_balance, max_positions)
        
        # Kucoin Futures fees (FIXED: Code Review)
        self.maker_fee = 0.0002  # 0.02% (TP orders)
        self.taker_fee = 0.0006  # 0.06% (SL and market orders)
        self.slippage = 0.0001  # 0.01% base slippage (FIXED from 0.1%)
        self.funding_rate = 0.0001  # 0.01% per 8h (KuCoin neutral rate)
    ...
    def close_position(self, position: Position, price: float, reason: str = "manual") -> bool:
        ...
        # Exit fee: FIXED - TP uses maker, SL uses taker (Code Review Fix #4)
        if reason == "take_profit":
            exit_fee = (position.size * fill_price) * self.maker_fee  # Limit order
        else:
            exit_fee = (position.size * fill_price) * self.taker_fee  # Market order (SL, manual)
...
class LivePositionManager(PositionManager):
    ...
    def __init__(self, initial_balance: float, kucoin_client, max_positions: int = 5):
        """
        Initialize live trading manager (FIXED: default max_positions 5).
