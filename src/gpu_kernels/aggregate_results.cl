...
__kernel void aggregate_flat_results(
    ...
    __global const int* data_signals,     // NEW: Signals for each data point
    const int num_bots,
    const int num_data_points,
    __global float* output                // Output: [trades, wins, pnl, signals] × num_bots
) {
    ...
    for (int i = 0; i < num_data_points; i++) {
        if (data_bot_ids[i] == my_bot_id) {
            ...
            total_signals += (float)data_signals[i];  // NEW
        }
    }
    
    // Write aggregated results
    int out_idx = bot_idx * 4;
    output[out_idx] = total_trades;
    output[out_idx + 1] = total_wins;
    output[out_idx + 2] = total_pnl;
    output[out_idx + 3] = total_signals;  // NEW
}